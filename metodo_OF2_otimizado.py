...
3) Função para a construção da matriz de coeficientes do método OF2 otimizado.
...
def metodo_OF2_otimizado(parametro, n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_amplitude_referencia_teste_janelado, vetor_fase_referencia_teste_janelado):
    ...
    vetor_pesos_amplitude_OF2_otimizado = vetor_incognitas_amplitude[:-3]
    ...
    vetor_pesos_pedestal_OF2_otimizado = vetor_incognitas_pedestal[:-3]
    ...
    for indice_linha in range(len(Matriz_Pulsos_Sinais_Teste_Janelado)):
        ...
        # A fase de referência é o elemento de índice indice_linha do vetor vetor_fase_referencia_teste_janelado.
        valor_fase_referencia_teste = vetor_fase_referencia_teste_janelado[indice_linha]
        
        # Cálculo da amplitude estimada pelo método OF2 otmizado.
        valor_amplitude_estimada = np.dot(vetor_pesos_amplitude_OF2_otimizado, vetor_pulsos_sinais_teste)
        
        # Cálculo do termo da amplitude versus a fase estimada pelo método OF2 otimizado.
        valor_amplitude_versus_fase_estimada = np.dot(vetor_pesos_amplitude_versus_fase_OF2_otimizado, vetor_pulsos_sinais_teste)
        
        # Cálculo da fase estimada.
        valor_fase_estimada = (valor_amplitude_versus_fase_estimada)/(valor_amplitude_estimada)
        
        # Cálculo do pedestal estimado pelo método OF2 otimizado.
        valor_pedestal_estimado = np.dot(vetor_pesos_pedestal_OF2_otimizado, vetor_pulsos_sinais_teste)

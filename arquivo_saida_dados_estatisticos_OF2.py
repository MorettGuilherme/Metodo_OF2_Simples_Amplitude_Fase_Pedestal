...
# Objetivo do código: geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação pelo método do Filtro Ótimo (OF 2).
...
1) Função para o cálculo dos dados estatísticos do erro de estimação pelo método do Filtro Ótimo (Optimal Filtering - OF 2).
...
titulo_programa = colored("Geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação da amplitude, fase, ou pedestal pelo método do Filtro Ótimo (Optimal Filtering - OF 2):\n", attrs=["bold"])
...
def arquivo_saida_dados_estatisticos_erro_parametro(parametro, n_ocupacao, n_janelamento, media_erro_parametro, var_erro_parametro, desvio_padrao_erro_parametro):
    ...
    pasta_saida = f"Dados_Estatisticos_OF2_{parametro}_OC"
    ...
    arquivo_saida = f"dados_estatisticos_OF2_janelamento_{n_janelamento}.txt"
...
def principal_arquivo_saida_dados_estatisticos_OF2():
    ...
    for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
        ...
        for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
            ...
            lista_erro_amplitude = metodo_OF2(parametro_amplitude, n_janelamento, Matriz_Dados_Pulsos_Teste, vetor_amplitude_referencia_teste, vetor_fase_referencia_teste, Matriz_Covariancia)
            lista_erro_fase = metodo_OF2(parametro_fase, n_janelamento, Matriz_Dados_Pulsos_Teste, vetor_amplitude_referencia_teste, vetor_fase_referencia_teste, Matriz_Covariancia)
            lista_erro_pedestal = metodo_OF2(parametro_pedestal, n_janelamento, Matriz_Dados_Pulsos_Teste, vetor_amplitude_referencia_teste, vetor_fase_referencia_teste, Matriz_Covariancia)
...
principal_arquivo_saida_dados_estatisticos_OF2()
